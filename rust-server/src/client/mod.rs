...
impl<F, C> Api<C> for Client<F> where
    ...
    fn category(&self, param_key: String, param_domain: i32, param_category: i32, param_parents: i32, context: &C) -> Box<Future<Item=CategoryResponse, Error=ApiError>> {

        // Query parameters
        let query_key = format!("key={key}&", key=param_key.to_string());
        let query_domain = format!("domain={domain}&", domain=param_domain.to_string());
        let query_category = format!("category={category}&", category=param_category.to_string());
        let query_parents = format!("parents={parents}&", parents=param_parents.to_string());


        let uri = format!(
            "{}/magicCashew/keepa/1.0.0/category?{key}{domain}{category}{parents}",
            self.base_path,
            key=utf8_percent_encode(&query_key, QUERY_ENCODE_SET),
            domain=utf8_percent_encode(&query_domain, QUERY_ENCODE_SET),
            category=utf8_percent_encode(&query_category, QUERY_ENCODE_SET),
            parents=utf8_percent_encode(&query_parents, QUERY_ENCODE_SET)
        );
                    ...
                    Box::new(
                        ...
                        .and_then(|body| str::from_utf8(&body)
                                             ...
                                             .and_then(|body|
                                                 ...
                                                 serde_json::from_str::<Vec<models::Category>>(body)
                                                     .map_err(|e| e.into())

                                             ))
                        .map(move |body|
                            CategoryResponse::SearchResultsMatchingCriteria(body)
                        )
    ...
    fn product(&self, param_key: String, param_domain: i32, param_asin: Option<String>, param_code: Option<String>, context: &C) -> Box<Future<Item=ProductResponse, Error=ApiError>> {

        // Query parameters
        let query_key = format!("key={key}&", key=param_key.to_string());
        let query_domain = format!("domain={domain}&", domain=param_domain.to_string());
        let query_asin = param_asin.map_or_else(String::new, |query| format!("asin={asin}&", asin=query.to_string()));
        let query_code = param_code.map_or_else(String::new, |query| format!("code={code}&", code=query.to_string()));


        let uri = format!(
            "{}/magicCashew/keepa/1.0.0/product?{key}{domain}{asin}{code}",
            self.base_path,
            key=utf8_percent_encode(&query_key, QUERY_ENCODE_SET),
            domain=utf8_percent_encode(&query_domain, QUERY_ENCODE_SET),
            asin=utf8_percent_encode(&query_asin, QUERY_ENCODE_SET),
            code=utf8_percent_encode(&query_code, QUERY_ENCODE_SET)
        );
            ...
            match response.status().as_u16() {
                200 => {
                    ...
                    Box::new(
                        ...
                        .map(move |body|
                            ProductResponse::SearchResultsMatchingCriteria(body)
                        )
                    ) as Box<Future<Item=_, Error=_>>
                },
                400 => {
