...
fn main() {
    ...
    match matches.value_of("operation") {
        ...
        Some("Category") => {
            let result = core.run(client.category("key_example".to_string(), 56, 56, 56));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },

        Some("Product") => {
            let result = core.run(client.product("key_example".to_string(), 56, Some("asin_example".to_string()), Some("code_example".to_string())));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
