...
public interface CategoryApi  {
    ...
    @ApiOperation(value = "Returns Amazon category information from Keepa API.", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "search results matching criteria", response = Category.class, responseContainer = "List"),
        @ApiResponse(code = 400, message = "bad input parameter") })
    public List<Category> category(@QueryParam("key") String key, @QueryParam("domain") Integer domain, @QueryParam("category") Integer category, @QueryParam("parents") Integer parents);
}
