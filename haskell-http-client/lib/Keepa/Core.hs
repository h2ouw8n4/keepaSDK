...
-- | display the config
instance P.Show KeepaConfig where
  show c =
    T.printf
...
-- | constructs a default KeepaConfig
--
-- configHost:
--
...
-- @"keepa/0.1.0.0"@
--
newConfig :: IO KeepaConfig
newConfig = do
    logCxt <- initLogContext
    return $ KeepaConfig
        { configHost = "https://virtserver.swaggerhub.com/magicCashew/keepa/1.0.0"
        , configUserAgent = "keepa/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => KeepaConfig -> auth -> KeepaConfig
addAuthMethod config@KeepaConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: KeepaConfig -> IO KeepaConfig
...
withStderrLogging :: KeepaConfig -> IO KeepaConfig
...
withNoLogging :: KeepaConfig -> KeepaConfig
...
data KeepaRequest req contentType res accept = KeepaRequest
  { rMethod  :: NH.Method   -- ^ Method of KeepaRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of KeepaRequest
  , rParams   :: Params -- ^ params of KeepaRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (KeepaRequest req contentType res accept) NH.Method
rMethodL f KeepaRequest{..} = (\rMethod -> KeepaRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (KeepaRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f KeepaRequest{..} = (\rUrlPath -> KeepaRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (KeepaRequest req contentType res accept) Params
rParamsL f KeepaRequest{..} = (\rParams -> KeepaRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (KeepaRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f KeepaRequest{..} = (\rAuthTypes -> KeepaRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => KeepaRequest req contentType res accept -> param -> KeepaRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: KeepaRequest req contentType res accept -> param -> KeepaRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: KeepaRequest req contentType res accept -> param -> KeepaRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method 
          ...
          -> KeepaRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = KeepaRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: KeepaRequest req contentType res accept -> [NH.Header] -> KeepaRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: KeepaRequest req contentType res accept -> [NH.HeaderName] -> KeepaRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => KeepaRequest req contentType res accept -> KeepaRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => KeepaRequest req contentType res accept -> KeepaRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: KeepaRequest req contentType res accept -> [NH.QueryItem] -> KeepaRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: KeepaRequest req contentType res accept -> WH.Form -> KeepaRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: KeepaRequest req contentType res accept -> NH.Part -> KeepaRequest req contentType res accept
...
_setBodyBS :: KeepaRequest req contentType res accept -> B.ByteString -> KeepaRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: KeepaRequest req contentType res accept -> BL.ByteString -> KeepaRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => KeepaRequest req contentType res accept -> P.Proxy authMethod -> KeepaRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: KeepaConfig
    -> a
    -> KeepaRequest req contentType res accept
    -> IO (KeepaRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> KeepaConfig
  -> IO (KeepaRequest req contentType res accept)
_applyAuthMethods req config@(KeepaConfig {configAuthMethods = as}) =
  foldlM go req as
  where
